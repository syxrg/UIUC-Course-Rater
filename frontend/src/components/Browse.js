...
const Browse = (props) => {
  ...
  const sortData = (a, b) => {
    if (trimmedQuery) {
      if (
        a.Subject.toLowerCase().includes(trimmedQuery) &&
        !b.Subject.toLowerCase().includes(trimmedQuery)
      )
        return -1;
      if (
        !a.Subject.toLowerCase().includes(trimmedQuery) &&
  ...
  const filteredData = query
    ? data
        ...
        .reduce((unique, row) => {
          return unique.some((u) => u.CRN === row.CRN)
            ? unique
            : [...unique, row];
        }, [])
        .sort(sortData)
  ...
  return (
    <div className="outer">
      ...
      <div className="search-stuff">
        <TextField
          placeholder="Search by subject / course name / course number"
                ...
                <Link to={`/${props.page}/${row.CRN}`}>
                  <button
                    ...
                    style={{
                      ...
                      height: "100%",
                      width: "100%",
                      border: buttonHoverStates[index]
                        ? "1px solid light-gray"
                        : "1px solid gray",
                      borderRadius: "10px",
                      background: "none",
                      boxShadow: buttonHoverStates[index]
                        ? "5px 5px 5px #C9C7C6"
                        : "none",
                    }}
